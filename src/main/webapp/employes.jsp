...
<head>
    ...
    <title>Gestion des Employ&eacute;s</title>
    <script>
        (function () {
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .pm-search-bar .pm-input { flex: 1 1 220px; }
...
<body class="pm-body">
    ...
    <nav class="navbar navbar-expand-lg pm-navbar mb-4">
        <div class="container">
            ...
            <div class="navbar-nav align-items-center">
                <a class="nav-link active" href="employes"> Employ&eacute;s</a>
    ...
    <div class="container my-4">
        <p class="pm-subtitle"> he consultez et mettez à jour votre équipe instantanément..</p>
        ...
        <div class="row g-4">
                ...
                <div class="pm-card pm-sticky">
                    <div class="card-body">
                        <h5 class="pm-section-title">
                            <c:choose>
                                <c:when test="${not empty employeEditer}">
                                    <span class="pm-kicker">&Eacute;dition</span> Modifier (${employeEditer.codeemp})
                                </c:when>
                                <c:otherwise>
                                    <span class="pm-kicker">Nouveau</span> Employ&eacute;
                                </c:otherwise>
                            </c:choose>
                        </h5>
                        <c:if test="${empty employeEditer}">
                            <p class="pm-subtitle" style="margin-top:-0.5rem;">Code g&eacute;n&eacute;r&eacute; automatiquement.</p>
                        </c:if>

                        <form action="employes" method="post">
                            <input type="hidden" name="codeemp" value="${employeEditer.codeemp}" />

                            <div class="mb-3">
                                <label for="nom" class="pm-label d-block">Nom</label>
                                <input type="text" name="nom" id="nom" class="form-control pm-input" value="${employeEditer.nom}" placeholder="Ex : Rakoto" required />
                            </div>
                            <div class="mb-3">
                                <label for="prenom" class="pm-label d-block">Pr&eacute;nom</label>
                                <input type="text" name="prenom" id="prenom" class="form-control pm-input" value="${employeEditer.prenom}" placeholder="Ex : Jean" required />
                            </div>
                            <div class="mb-3">
                                <label for="poste" class="pm-label d-block">Poste</label>
                                <input type="text" name="poste" id="poste" class="form-control pm-input" value="${employeEditer.poste}" placeholder="Ex : D&eacute;veloppeur" required />
                            </div>

                            <div class="d-grid gap-2 mt-4">
                                <button type="submit" class="btn pm-btn ${not empty employeEditer ? 'pm-btn-edit' : 'pm-btn-gold'}">
                                    ${not empty employeEditer ? 'Modifier l\'employé' : 'Enregistrer l\'employé'}
                                </button>
                                <c:if test="${not empty employeEditer}">
                                    <a href="employes" class="btn pm-btn pm-btn-ghost">Annuler</a>
                                </c:if>
                            </div>
                        </form>
                    </div>
            ...
            <!-- ===== Colonne DROITE : Recherche + Tableau ===== -->
            <div class="col-lg-8">

                <!-- Barre de recherche -->
                <form action="employes" method="get" class="pm-search-bar">
                    <input type="text" name="recherche" class="form-control pm-input" placeholder="Rechercher par code, nom, pr&eacute;nom ou poste..." value="${recherche}" />
                    <button type="submit" class="btn pm-btn pm-btn-gold">Rechercher</button>
                    <c:if test="${not empty recherche}">
                        <a href="employes" class="btn pm-btn pm-btn-outline">Actualiser</a>
                    </c:if>
                </form>

                <!-- Tableau des employés -->
                <h4 class="pm-section-title"><span class="pm-kicker">Liste</span> Employ&eacute;s</h4>
                <div class="pm-table-wrap">
                    <table class="pm-table">
                        <thead>
                            <tr>
                                <th>Code Employ&eacute;</th>
                                <th>Nom</th>
                                <th>Pr&eacute;nom</th>
                                <th>Poste</th>
                                <th>Actions</th>
                        ...
                        <tbody>
                            <c:forEach var="emp" items="${employes}">
                                <tr>
                                    <td class="pm-code-cell">${emp.codeemp}</td>
                                    <td>${emp.nom}</td>
                                    <td>${emp.prenom}</td>
                                    <td>${emp.poste}</td>
                                    <td>
                                        <div class="pm-actions">
                                            <a href="employes?action=editer&codeemp=${emp.codeemp}" class="btn pm-btn pm-btn-sm pm-btn-edit">Modifier</a>
                                            <!-- Bouton Supprimer avec attribut data-href -->
                                            <a href="#" data-href="employes?action=supprimer&codeemp=${emp.codeemp}" class="btn pm-btn pm-btn-sm pm-btn-del btn-supprimer">Supprimer</a>
                                        </div>
                                    </td>
                                </tr>
                            </c:forEach>
                            <c:if test="${empty employes}">
                                <tr>
                                    <td colspan="5" class="pm-empty">Aucun employé trouv&eacute;.</td>
                                </tr>
                            </c:if>
                        </tbody>
                    </table>
    ...
    <div class="pm-modal-overlay" id="pmConfirmOverlay" role="dialog" aria-modal="true">
        <div class="pm-modal" data-variant="error">
            ...
            <p class="pm-modal-text">&Ecirc;tes-vous s&ucirc;r de vouloir supprimer cet &eacute;l&eacute;ment ? Cette action est irr&eacuteversible.</p>
    ...
    <!-- ===== Modal de feedback (Succès / Erreur) ===== -->
    ...
    <!-- Gestion de la modale de feedback (Succès / Erreur) -->
    <script>
        (function () {
